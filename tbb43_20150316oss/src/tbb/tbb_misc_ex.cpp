...
static void initialize_hardware_concurrency_info () {
    ...
    dynamic_link( L"Kernel32.dll", ProcessorGroupsApiLinkTable,
	...
	PrintExtraVersionInfo(L"Processor groups", L"%d", ProcessorGroupInfo::NumGroups);
    if (ProcessorGroupInfo::NumGroups>1)
        for (int i=0; i<ProcessorGroupInfo::NumGroups; ++i)
			PrintExtraVersionInfo(L"----- Group", L"%d: size %d", i, theProcessorGroups[i].numProcs);
}
