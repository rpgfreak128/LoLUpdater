...
void PrintExtraVersionInfo( const wchar_t* category, const wchar_t* format, ... ) {
    if( PrintVersionFlag ) {
        wchar_t str[1024]; memset(str, 0, 1024);
        va_list args; va_start(args, format);
        // Note: correct vsnprintf definition obtained from tbb_assert_impl.h
        _vsnwprintf( str, 1024-1, format, args);
        va_end(args);
        fwprintf(stderr, L"TBB: %s\t%s\n", category, str );
    }
}

void PrintRMLVersionInfo( void* arg, const wchar_t* server_info ) {
    PrintExtraVersionInfo( server_info, (const wchar_t *)arg );
}
