...
#define ATOMIC_ONCE_DECL( var ) pthread_once_t var = PTHREAD_ONCE_INIT
    ...
    /*
        ...
        ret      -- 0         -- Error occurred.
                    ...
                    buffer.
    */
    static size_t abs_path( wchar_t const * name, wchar_t * path, size_t len ) {
        if ( !ap_data._len )
            return 0;
    ...
    bool dynamic_link( const wchar_t* library, const dynamic_link_descriptor descriptors[], size_t required, dynamic_link_handle *handle, int flags ) {
        init_dynamic_link_data();
